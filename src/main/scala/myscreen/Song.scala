...
object Song
{
	def baseDir = new File("/Users/Shared/StThomas/songs");

	def apply(show : SlideShow, name : String) : Song =
	{
		apply(show = show, file = new File(baseDir, name + ".txt"))
	}
	...
	def list : Seq[File] =
	{
		...
		baseDir.listFiles(filter).toSeq.sortWith(_.getName < _.getName)
	}
