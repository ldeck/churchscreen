...
object Slide
{
	...
	def bibleReading = (s:Slide) =>
	{
		...
		s.picSlide(pngName = "stthomas-biblereading.png", r = new Rectangle(p, d))
	...
	{
		s.blank(Color.WHITE)
		s.picSlide("stthomas-exit-1030.png")
	}

	def welcome = (s:Slide) =>
	{
		s.blank(Color.WHITE)
		s.picSlide("stthomas-welcome-1030.png")
...
class Slide(val show : SlideShow, val slide : HSlide)
{
	...
	private[Slide] def picFile(name : String) : File =
	{
		...
		val picturesHome = new File(userHome, "Pictures")
		val png = new File(picturesHome, name)
		if (!png.isFile)
		{
			throw new FileNotFoundException("No file named %s in Pictures".format(name))
		}
		return png
	}
}
